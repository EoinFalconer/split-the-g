...
import {PourLoader, PintsSign, withDot} from '@/components/Brand'
...
function getPlayerName(slug: string): string {
  try {
    const saved = window.localStorage.getItem(`stg-player-${slug}`)
...
function PostCard({post, onLike}: {post: Post; onLike: (post: Post) => void}) {
  ...
  const aspect = post.imgW && post.imgH ? `${post.imgW} / ${post.imgH}` : '3 / 4'
  ...
  return (
    <article className="overflow-hidden rounded-2xl border-[1.5px] border-ink-faint bg-white/45">
      <header className="flex items-center gap-3 px-4 py-3">
        ...
        <span className={`flabel shrink-0 ${post.split ? 'text-coral' : 'text-ink-soft'}`}>
          {!post.split
            ? 'miss'
            : post.mode === 'dropHarp'
              ? 'harp dropped!'
      ...
      <div className="flex items-center gap-2 px-4 pt-3">
        ...
        {post.score != null && (
          <span className="ml-auto flex flex-col items-end">
            ...
            <span className="text-[10px] leading-tight text-ink-soft">{perfectLabel(post.mode)}</span>
          </span>
        )}
      </div>
...
export function Feed({event}: {event: EventConfig}) {
  ...
  const playHref = `/e/${event.slug}/play`
  const scope = event.id ? `&event=${event.id}` : ''
  ...
  const refresh = useCallback(async () => {
    const res = await fetch(`/api/feed?device=${deviceRef.current}${scope}`)
    if (!res.ok) return
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/feed?device=${deviceRef.current}${scope}&before=${encodeURIComponent(last.createdAt)}`,
      )
      if (!res.ok) return
      const {items, hasMore: more} = await res.json()
  ...
  const toggleLike = useCallback(async (post: Post) => {
    ...
    try {
      const res = await fetch(`/api/attempts/${post._id}/like`, {
        ...
        body: JSON.stringify({deviceId: deviceRef.current, name: getPlayerName(event.slug)}),
      })
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col pb-28">
      <header className="sticky top-0 z-10 border-b border-ink-faint/50 bg-paper/85 backdrop-blur">
        <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <Link href={playHref} className="fbtn fbtn-sm">
            pour one
          </Link>
        </div>
      </header>
      ...
      {posts !== null && posts.length === 0 && (
        <section className="flex flex-col items-center gap-6 px-5 py-16 text-center">
          <p className="text-2xl italic text-ink-mid">Not a single pint on the wall yet.</p>
          <Link href={playHref} className="fbtn">
            pour the first one
          </Link>
        </section>
      )}
