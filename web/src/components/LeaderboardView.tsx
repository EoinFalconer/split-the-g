...
export function LeaderboardView({event}: {event: EventConfig}) {
  const hasWindow = Boolean(event.startsAt && event.endsAt)
  const started = event.startsAt ? new Date().toISOString() >= event.startsAt : false
  const [players, setPlayers] = useState<Row[]>([])
  const [latest, setLatest] = useState<Latest>(null)
  const [recent, setRecent] = useState<Recent[]>([])
  // With a window, open on the championship once it has begun; otherwise the
  // warm-up board. Without one, there's just a single board.
  const [board, setBoard] = useState<Board>(() =>
    hasWindow && started ? 'championship' : hasWindow ? 'practice' : 'championship',
  )
  ...
  useEffect(() => {
    ...
    if (event.startsAt) params.set('from', event.startsAt)
    if (event.endsAt) params.set('to', event.endsAt)
    const load = async () => {
      try {
  ...
  }, [board, event.id, event.startsAt, event.endsAt])
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col items-center gap-8 px-5 py-8 pb-28 sm:gap-10 sm:px-10 sm:py-12 sm:pb-28">
      <header className="flex flex-col items-center gap-1 text-center">
        {event.hello && <p className="hello">{withDot(event.hello)}</p>}
        ...
        {hasWindow && (
          <>
            <div className="mt-5 flex items-center gap-2 rounded-full border-[1.5px] border-ink-faint bg-white/45 p-1">
              {(
                [
                  {key: 'championship', label: event.championshipLabel},
                  {key: 'practice', label: event.practiceLabel},
            ...
            <p className="mt-2 text-sm italic text-ink-mid">
              {board === 'championship'
                ? `only pints poured ${event.dateLabel ? `on ${event.dateLabel}` : 'during the event'} count here`
                : 'every warm-up pint outside the main event'}
            </p>
          </>
        )}
      </header>
      ...
      <table className="w-full max-w-4xl border-separate border-spacing-y-1 text-lg sm:text-2xl">
        ...
        <tbody>
          {players.map((p, i) => (
            <tr key={p._id} className={i === 0 ? 'bg-wash text-ink' : 'text-ink-deep'}>
              <td className="rounded-l-xl px-4 py-3 tabular-nums">{i === 0 ? '🏆' : i + 1}</td>
          ...
          {players.length === 0 && (
            <tr>
              <td colSpan={7} className="py-16 text-center text-2xl italic text-ink-soft">
                {board === 'championship' && hasWindow
                  ? 'The championship board is bare — first clean pint takes the early lead.'
      ...
      <footer className="mt-auto flex flex-col items-center gap-4 pt-6">
        <Link href={`/e/${event.slug}`} className="flabel underline decoration-ink-faint underline-offset-8">
          back to the feed
        </Link>
        <PintsSign signoff={event.signoff} />
      </footer>
      <BottomNav slug={event.slug} />
    </main>
  )
}
